 ...
 * - 支持8/32/256优先级，其中0表示最高，7/31/255表示最低。最低优先级7/31/255用于空闲线程。
 ...
 * 另外，根据定时器超时函数执行时所处的上下文环境，RT-Thread的定时器可以分为硬件定时器和软件定时器。
 *
 */
 ...
 * - 信号量释放操作对中断服务程序是安全的。
 *
 * 此外，线程获取信号量或互斥量的阻塞队列可以按优先级或FIFO排序。有两个标志来表示这种机制。
 * - RT_IPC_FLAG_FIFO
