 ...
 *
 * INIT_BOARD_EXPORT(fn)声明的函数会被rt_components_board_init()调用，
 * 其他宏声明的函数会被rt_components_init()调用。
 *
 * RT-Thread的启动流程如下图所示：
 *
 ...
 * 这个周期性硬件定时器称之为系统时钟。时钟节拍（OS Tick）是RT-Thread 操作系统中最小的时钟单位，
 * 系统节拍一般定义为32位无符号整数，提供给应用程序所有和时间有关的服务，
 * 如线程的延时、线程的时间片轮转调度以及定时器超时等，从系统启动开始计数的时钟节拍数称为系统时间。
 * 时钟节拍来源于定时器的周期性中断，一次中断表示一个OS Tick。
 * OS Tick的长度可以根据RT_TICK_PER_SECOND的定义来调整，
 * 等于1/RT_TICK_PER_SECOND秒，精度越高的时钟将导致系统中定时器频繁检查。
 */
 ...
 * @brief 定时器管理接口
 *
 * RT-Thread的定时器提供两类定时器机制：第一类是单次触发定时器，这类定时器在启动后只会触发一次定时器事件，
 * 然后定时器自动停止。第二类是周期触发定时器，这类定时器会周期性的触发定时器事件，
 * 直到用户手动的停止，否则将永远持续执行下去。
 *
 * 另外，根据超时函数执行时所处的上下文环境，RT-Thread的定时器可以分为软件定时器和硬件定时器。
 * 硬件定时器的超时函数在中断上下文环境中执行，可以在初始化/创建定时器时使用参数RT_TIMER_FLAG_HARD_TIMER来指定。
 * 软件定时器的超时函数在timer线程的上下文环境中执行。可以在初始化/创建定时器时使用参数RT_TIMER_FLAG_SOFT_TIMER来指定。
 *
 */
