 ...
 * @brief 初始化内存池
 *
 * 该函数将初始化一个静态的内存池对象。内存块个数 = size / (block_size + 4链表指针大小)，计算结果取整数。
 * 例如：内存池数据区总大小 size为4096字节，内存块大小block_size设为80字节；则申请的内存块个数为 4096/ (80+4) = 48个。
 *
 * @param mp 内存池对象句柄
 * @param name 内存池的名称
 ...
 * @brief 脱离内存池
 *
 * 该函数将把静态内存池对象从内核对象管理器中脱离。
 * 调用改函数内核先唤醒所有等待在该内存池对象上的线程，然后将内存池对象从
 * 内核对象管理器中脱离。
 *
 * @param mp 内存池对象
 *
 ...
 * @brief 创建内存池
 *
 * 调用该函数将会创建一个动态内存池对象并从堆上分配一个内存池。
 *
 * @param name 内存池的名称
 * @param block_count 内存块数量
 * @param block_size 内存块容量
 *
 * @return 成功返回创建的内存池的句柄； 创建失败返回RT_NULL
 */
 ...
 * @brief 删除内存池
 *
 * 该函数将删除动态内存池对象并释放申请的内存。删除内存池时，会首先唤醒等待在该内存池对象上的
 ...
 * 空闲内存块，则判断超时时间：若超时时间设置为零，则立刻返回空内存块；若等待
 * 时间大于零，则把当前线程挂起在该内存池对象上，直到内存池中有可用的内存块，或等待时间到达。
 *
 * @param mp 内存池对象
 * @param time 超时时间，单位为系统时钟节拍
 *
 * @return 成功则返回分配的内存块地址，失败则返回 RT_NULL
 */
 ...
 * @brief 释放内存块
 *
 * 任何内存块使用完后都必须被释放，否则可能会造成内存泄露。调用该函数释放指定的内存块，
