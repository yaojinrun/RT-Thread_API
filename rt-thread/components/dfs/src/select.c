 ...
 * @brief 监视I/O设备状态
 *
 * 该函数接口可以阻塞地同时探测一组支持非阻塞的I/O设备是否有事件发生（如可读，可写，有高优先级
 * 的错误输出，出现错误等等），直至某一个设备触发了事件或者超过了指定的等待时间。
 *
 * @param nfds 集合中所有文件描述符的范围，即所有文件描述符的最大值加1
 * @param readfds 需要监视读变化的文件描述符集合
 * @param writefds 需要监视写变化的文件描述符集合
 * @param exceptfds 需要监视出现异常的文件描述符集合
 * @param timeout select的超时时间
 *
 * @return 正值 监视的文件集合出现可读写事件或出错; 0 等待超时，没有可读写或错误的文件; 负值 select出现错误。
 */
