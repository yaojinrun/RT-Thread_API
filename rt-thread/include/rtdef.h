...
/*typedef rt_base_t                       rt_flag_t;      /**< @brief 参数类型(没有使用，不添加到文档) */
 ...
 * @brief 设备自动初始化
 *
 * 这里通常会运行一些设备相关的应用初始化函数，例如：串口通讯设置，...
 *
 * @param fn 函数指针
 *
 */
 ...
 */
#define RT_OBJECT_FLAG_MODULE           0x80            /**< @brief 动态模块对象 */
...
#define RT_TIMER_FLAG_ONE_SHOT          0x0             /**< @brief 单次定时器 */
#define RT_TIMER_FLAG_PERIODIC          0x2             /**< @brief 周期性定时器 */

#define RT_TIMER_FLAG_HARD_TIMER        0x0             /**< @brief 硬件定时器，定时器的回调函数将在tick isr中调用。 */
...
#define RT_TIMER_CTRL_SET_ONESHOT       0x2             /**< @brief 将定时器更改为一次性定时 */
#define RT_TIMER_CTRL_SET_PERIODIC      0x3             /**< @brief 将定时器更改为周期性定时 */
...
/**
 * 在 RT-Thread操作系统中，定时器控制块由结构体struct rt_timer定义并形成定时器内核对象，
 * 再链接到内核对象容器中进行管理。是操作系统用于管理定时器的一个数据结构，它会存储定时器的一些信息，
 * 例如初始节拍数，超时时的节拍数，也包含定时器与定时器之间连接用的链表结构，超时回调函数等。
 *
 * @brief 定时器结构体定义
 *
 */
...
struct rt_thread
{
    ...
    struct rt_timer thread_timer;                       /**< @brief 线程的内置定时器 */
 ...
 * IPC标志和控制命令定义
 */
#define RT_IPC_FLAG_FIFO                0x00            /**< @brief 先进先出方式等待资源  */
#define RT_IPC_FLAG_PRIO                0x01            /**< @brief 优先级方式等待资源  */

/* #define RT_IPC_CMD_UNKNOWN              0x00            /**< @brief 位置的 IPC 命令[没有使用，拿掉] */
...
struct rt_mutex
{
    ...
    rt_uint8_t           original_priority;             /**< @brief 持有线程的原始优先级 */
    rt_uint8_t           hold;                          /**< @brief 持有线程的持有次数 */

    struct rt_thread    *owner;                         /**< @brief 当前持有该互斥量的线程 */
};
/**
 * @brief 互斥量类型指针定义
 */
...
struct rt_mailbox
{
    ...
    rt_uint32_t         *msg_pool;                      /**< @brief 邮箱缓冲区的起始地址 */

    rt_uint16_t          size;                          /**< @brief 邮箱缓冲区大小 */

    rt_uint16_t          entry;                         /**< @brief 邮箱中邮件的数目 */
    rt_uint16_t          in_offset;                     /**< @brief 邮箱缓冲区的输入偏移量 */
    rt_uint16_t          out_offset;                    /**< @brief 邮箱缓冲区的输出偏移量 */
...
struct rt_messagequeue
{
    ...
    rt_uint16_t          entry;                         /**< @brief 队列中的已有消息数 */

    void                *msg_queue_head;                /**< @brief 消息队列链表头 */
    void                *msg_queue_tail;                /**< @brief 消息队列链表尾 */
    void                *msg_queue_free;                /**< @brief 消息队列空闲的链表 */
};
/**
 * @brief 消息队列类型指针定义
 */
...
struct rt_memheap_item
{
    ...
    struct rt_memheap      *pool_ptr;                   /**< @brief 内存堆句柄 */
...
struct rt_memheap
{
    ...
    rt_uint32_t             available_size;             /**< @brief 可以使用的大小 */
    rt_uint32_t             max_used_size;              /**< @brief 最大可使用的大小 */
...
struct rt_mempool
{
    ...
    rt_list_t        suspend_thread;                    /**< @brief 因内存块不可用挂起的线程链表 */
